...
static void ImImpl_RenderDrawLists(ImDrawList** const cmd_lists, int cmd_lists_count)
{
    ...
    glUseProgram(g_imgui_program);
    GLint texture_location  = glGetUniformLocation(g_imgui_program, "sampler");
    GLint proj_mtx_location = glGetUniformLocation(g_imgui_program, "proj");
    ...
    if (neededBufferSize > (size_t)g_imgui_vbo_size)
    {
        g_imgui_vbo_size = neededBufferSize + 5000;  // Grow buffer
        glBufferData(GL_ARRAY_BUFFER, (GLsizeiptr)g_imgui_vbo_size, NULL, GL_STREAM_DRAW);
    }
